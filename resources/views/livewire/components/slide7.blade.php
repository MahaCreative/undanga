    ...
    class="absolute top-52 md:top-40 lg:top-36 py-2.5 px-4 my-3 mb-4 bg-white bg-opacity-75 shadow-md shadow-primary text-center rounded-lg backdrop-blur-md">
        ...
        <form wire:ignore.self wire:submit.prevent="submitHandler" action="">
            ...
            <div class="my-2 flex flex-col items-start">
                ...
                <input type="text" wire:model="nama_pemilik" name="nama_pemilik" class="text-primary outline-none rounded-md border shadow-sm w-full py-1 px-3 shadow-primary hover:outline-dashed hover:outline-primary focus:outline-dashed focus:outline-primary focus:ring-primary ">
                 @error('nama_pemilik')
                    <p class="text-red-600 italic text-sm">{{$message}}</p>
                @enderror
            </div>
            <div class="my-2 flex flex-col items-start">
                ...
                <input type="text" wire:model="pesan" name="pesan" class="text-primary outline-none rounded-md border shadow-sm w-full py-1 px-3 shadow-primary hover:outline-dashed hover:outline-primary focus:outline-dashed focus:outline-primary focus:ring-primary ">
                 @error('pesan')
                    <p class="text-red-600 italic text-sm">{{$message}}</p>
                @enderror
            </div>
            <div class="my-2 flex flex-col items-start">
                ...
                <input type="number" wire:model="nominal"name="nominal" class="text-primary outline-none rounded-md border shadow-sm w-full py-1 px-3 shadow-primary hover:outline-dashed hover:outline-primary focus:outline-dashed focus:outline-primary focus:ring-primary ">
                 @error('nominal')
                    <p class="text-red-600 italic text-sm">{{$message}}</p>
                @enderror
            </div>
            <button type="submit" class="w-full text-white  text-sm italic rounded-bl-lg rounded-br-lg bg-primary py-2">Lanjutkan</button>
        </form>
    </div>
</div>